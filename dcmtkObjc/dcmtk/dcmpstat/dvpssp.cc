...
OFCondition DVPSStoredPrint::printSCUprintBasicFilmBox(DVPSPrintMessageHandler& printHandler)
{
  ...
  OFCondition cond = printHandler.actionRQ(UID_BasicFilmBoxSOPClass, filmBoxInstanceUID.c_str(), 1 , NULL, status, attributeListOut);//1=print
...
OFCondition DVPSStoredPrint::printSCUprintBasicFilmSession(DVPSPrintMessageHandler& printHandler)
{
  ...
  OFCondition cond = printHandler.actionRQ(UID_BasicFilmSessionSOPClass, filmSessionInstanceUID.c_str(),1 , NULL , status, attributeListOut);//1=print
  delete attributeListOut; // should be empty anyway
        ...
        if (transmitImagesIn12Bit)
        {
          ...
          if (EC_Normal==result)
          {
             ...
             if (pxData && pxDataVoid)
             {
               ...
               if (EC_Normal==result) result = ditem->insert(pxData, OFTrue); //replaceOld
               else delete pxData;
  ...
  DcmUniqueIdentifier& imgSeries)
{
  ...
  if (result && rqDataset)
  {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub))
    {
      ...
      else if (currentTag == DCM_ImageDisplayFormat);
      else if (currentTag == DCM_FilmOrientation);
      else if (currentTag == DCM_FilmSizeID);
      else if (currentTag == DCM_MagnificationType);
      else if (currentTag == DCM_SmoothingType);
      else if (currentTag == DCM_BorderDensity);
      else if (currentTag == DCM_EmptyImageDensity);
      else if (currentTag == DCM_MinDensity);
      else if (currentTag == DCM_MaxDensity);
      else if (currentTag == DCM_Trim);
      else if (currentTag == DCM_ConfigurationInformation);
      else if (currentTag == DCM_RequestedResolutionID);
      else if (currentTag == DCM_ReferencedFilmSessionSequence);
  ...
  if (result)
  {
    ...
    if (rspDataset)
    {
      ...
      {      
        ...
        if (newRefFilmSessionSequence) rspDataset->insert(newRefFilmSessionSequence, OFTrue);
  ...
  DVPSPresentationLUT_PList& globalPresentationLUTList)
{
  ...
  if (result && rqDataset)
  {
    ...
    while (EC_Normal == rqDataset->nextObject(stack, intoSub))
    {
      intoSub = OFFalse;
      const DcmTagKey& currentTag = (stack.top())->getTag();
      if (currentTag.getElement() == 0x0000);//groupLength
      else if (currentTag == DCM_MagnificationType);
      else if (currentTag == DCM_SmoothingType);
      else if (currentTag == DCM_BorderDensity);
      else if (currentTag == DCM_EmptyImageDensity);
